...
export function ReservationsClient({
  ...
  const upcoming = filtered.filter(
    (reservation) => new Date(reservation.checkInDate) >= today && reservation.status !== 'CANCELLED'
  )
  const past = filtered.filter(
    (reservation) => new Date(reservation.checkInDate) < today || reservation.status === 'CHECKED_OUT'
  )
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardContent className="p-5">
            <p className="text-sm text-muted-foreground">Hospedes ativos</p>
            <p className="mt-1 text-2xl font-bold text-blue-600">{active.length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            <p className="text-sm text-muted-foreground">Proximos check-ins</p>
      ...
      <div className="space-y-3">
        {filtered.map((reservation) => (
          <Card key={reservation.id} className="transition-shadow hover:shadow-card-hover">
              ...
              <div className="min-w-0 flex-1 space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1.5">
                    ...
                    {reservation.numberOfGuests} hospede{reservation.numberOfGuests > 1 ? 's' : ''}
                  </span>
                </div>
