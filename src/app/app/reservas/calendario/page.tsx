...
export default async function CalendarPage({
  ...
  const checkInsThisMonth = reservations.filter((reservation) => {
    const date = new Date(reservation.checkInDate)
    return date.getMonth() === currentMonth && date.getFullYear() === currentYear
  }).length

  const checkOutsThisMonth = reservations.filter((reservation) => {
    const date = new Date(reservation.checkOutDate)
    return date.getMonth() === currentMonth && date.getFullYear() === currentYear
  }).length

  const occupiedDays = cells.filter((cell) => cell.day && cell.reservations.length > 0).length

  const metrics = [
    {
      title: 'Reservas no mes',
      value: reservations.length,
      hint: 'Entradas que tocam este calendario',
      icon: CalendarDays,
      tone: 'brand' as const,
    },
    {
      ...
      hint: 'Chegadas previstas no periodo',
      icon: LogIn,
    ...
    },
    {
      ...
      hint: 'Saidas programadas no periodo',
      icon: LogOut,
      tone: 'blue' as const,
    },
    {
      title: 'Dias ocupados',
      value: occupiedDays,
      hint: 'Dias com pelo menos uma reserva',
      icon: CheckCircle2,
      tone: 'slate' as const,
    },
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      <DashboardSectionCard
        ...
        description="Use a grade para localizar reservas rapidamente, entender a densidade do mes e abrir cada estadia sem sair da leitura visual."
        action={
          <Badge variant="outline" className="bg-background">
            {occupiedDays} dia{occupiedDays === 1 ? '' : 's'} ocupado{occupiedDays === 1 ? '' : 's'}
          </Badge>
        }
      ...
      >
        {reservations.length === 0 ? (
          <EmptyState
            icon={CalendarDays}
            title="Nenhuma reserva neste mes"
            description="Cadastre reservas para visualizar ocupacao, check-ins e check-outs diretamente no calendario."
            actionLabel="Nova reserva"
            actionHref="/app/reservas/novo"
            secondaryActionLabel="Ver lista"
            secondaryActionHref="/app/reservas"
          />
        ) : (
          <>
                  ...
                  <div key={weekIndex} className="grid grid-cols-7 divide-x">
                    {week.map((cell, cellIndex) => (
                      <div
                        key={cellIndex}
                        className={cn(
                          'min-h-[108px] p-1.5 sm:min-h-[132px] sm:p-2',
            ...
            <div className="space-y-3">
              <p className="text-sm font-medium text-foreground">Legenda de status</p>
              <div className="flex flex-wrap items-center gap-3 text-xs">
                {[
                  ...
                  {
                    label: 'Cancelada',
                    className: 'bg-rose-100 text-rose-700 border-rose-200 line-through opacity-60',
                  },
                ].map((item) => (
                  <div
                    key={item.label}
